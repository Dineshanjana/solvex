...
async function handleLinkedInAuth(req, res, userId) {
  ...
  try {
    ...
    const { sub } = profileRes.data;
    const linkedInId = sub

    const existingLinkedIn = await prisma.fbAuth.findUnique({
      where: { fbId: linkedInId },
    });

    const LinPage = await prisma.fbPage.findUnique({
      where: { PageId: linkedInId },
    })
...
async function handleXAuth(req, res) {
  ...
  try {
    ...
    const existingX = await prisma.fbAuth.findUnique({
      where: { fbId: twitterId },
    });

    const existingPage = await prisma.fbPage.findUnique({
      where: { PageId: twitterId },
    });
